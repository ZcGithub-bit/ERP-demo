...
Dialog.prototype.create = function () {
  ...
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: none; font-size:12px; line-height:1.4;">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "") + '">\
      ...
      <td height="23" style="background-color: #035bb1;"><div style="padding: 9px 0 0 4px; float: left; font-weight: bold; color:#fff;margin-top:7px;"><span style="font-size:16px;"  id="_Title_' + this.ID + '">' + this.Title + '</span></div>\
        <div onclick="Dialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(Dialog.getInstance(\'' + this.ID + '\').cancelButton,[]);"  style="margin-top:2px; position: relative; cursor: pointer; float: right; color:#fff;font-size:20px;' + (this.ShowCloseButton ? "" : "display:none;") + '">X&nbsp;&nbsp;&nbsp;</div></td>\
...
function showImage (src) {
	...
	if(img){
		diagImg = new Dialog();
		diagImg.Width=img.width;
		diagImg.Height=img.height;
		diagImg.InnerHtml="<img onclick='diagImg.close();' src= '"+src+"' />";
		diagImg.Title="查看大图";
//		diagImg.OKEvent = function(){diag.close();};//点击确定后调用的方法
		diagImg.show();
	}
}
function showMsg (msg,func){
	Dialog.alert("<br/><span style='color:#035bb1'>"+msg+"</span><br/>",func);
}